...
v_result = dbutils.notebook.run("(Clone) 1.ingest_circuits_file", 0, {"P_data_source": "Ergast API"})

# COMMAND ----------

v_result = dbutils.notebook.run("(Clone) 1.ingest_circuits_file", 0, {"P_data_source": "Ergast API"})

# After running the above code, if it fails, navigate to the job runs view in Databricks.
# Click on the link in the Start time column for the run to view detailed error information.
